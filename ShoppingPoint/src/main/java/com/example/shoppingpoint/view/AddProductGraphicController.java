...
public class AddProductGraphicController {
    ...
    public void initialize() throws Exception {
        switch (((StoreOwner) LoggedInUser.getInstance().getUser()).getStore().getType()) {
            case CLOTHES -> {
                typeComboBox.getItems().addAll("Clothes", "Shoes");
            }
            case BOOKS -> {
                typeComboBox.getItems().addAll("Book", "Comics");
            }
            case VIDEOGAMES -> {
                typeComboBox.getItems().addAll("Video Game", "Game Console");
            }
            case ELECTRONICS -> {
                typeComboBox.getItems().addAll("Home Appliances", "Computer");
            }
    ...
    public void goNext(ActionEvent actionEvent) throws Exception {
        ...
        switch (typeComboBox.getValue()) {
            ...
            case "Comics" -> {
                type = ProductType.COMICS;
            }
            case "Video Game" -> {
                type = ProductType.VIDEOGAME;
            }
            case "Game Console" -> {
                type = ProductType.GAMECONSOLE;
            }
